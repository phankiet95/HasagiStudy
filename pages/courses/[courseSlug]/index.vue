<template>
  ...
  <Teleport to="body">
    ...
    >
      ...
      >
        <!-- Backdrop -->
        <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" @click="deleteDialog.show = false" />

        <!-- Panel -->
        <div class="relative w-full max-w-sm bg-white rounded-3xl shadow-2xl overflow-hidden">
          <!-- Top accent -->
          <div class="h-1 bg-gradient-to-r from-red-400 to-red-500" />

          <div class="p-6">
            <!-- Icon -->
            <div class="w-12 h-12 rounded-2xl bg-red-50 flex items-center justify-center mb-4">
              <svg class="w-6 h-6 text-red-500" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
              </svg>
            </div>

            <!-- Text -->
            <h3 class="text-base font-bold text-gray-900 mb-1">Xóa khỏi bộ nhớ offline?</h3>
            <p class="text-sm text-gray-500 leading-relaxed">
              Bài <span class="font-semibold text-gray-700">"{{ deleteDialog.lesson?.title }}"</span>
              sẽ bị xóa. Bạn vẫn có thể xem lại khi có mạng.
            </p>
            ...
            <div class="flex gap-3 mt-5">
              ...
              <button
                @click="confirmDelete"
                class="flex-1 py-3 rounded-xl bg-red-500 hover:bg-red-600 text-white text-sm font-semibold transition-colors shadow-sm"
              >
                Xóa
              </button>
            </div>
...
const confirmDelete = async () => {
  ...
  const key = `${store.orgSlug}/${route.params.courseSlug}/${deleteDialog.lesson.slug}`
