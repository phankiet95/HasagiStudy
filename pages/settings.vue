    ...
    <div class="px-4 py-5 space-y-4">
      ...
      <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
        <div class="flex items-center gap-4">
          <div class="w-16 h-16 rounded-full overflow-hidden bg-gradient-to-br from-indigo-400 to-purple-400 flex items-center justify-center flex-shrink-0">
          ...
          <div class="flex-1 min-w-0">
            ...
            <span class="inline-flex items-center gap-1 mt-1 text-xs font-semibold text-indigo-600 bg-indigo-50 px-2.5 py-0.5 rounded-full">
              Học viên
            </span>
          </div>
        </div>
      ...
      <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
        ...
        <div class="flex items-center gap-3">
          <div class="w-12 h-12 rounded-xl overflow-hidden bg-indigo-50 border border-indigo-100 flex-shrink-0 flex items-center justify-center">
            <img v-if="store.orgLogo" :src="store.orgLogo" class="w-full h-full object-contain" />
            <span v-else class="text-xl font-black text-indigo-400">{{ store.orgName?.[0]?.toUpperCase() }}</span>
          </div>
          <div class="flex-1 min-w-0">
